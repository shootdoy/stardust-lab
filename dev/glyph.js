 ...
 * 폰트 base64 해시가 바뀌면 «목록을 다시 뽑으라»고 알린다. 다시 뽑는 법은 아래 REGEN 참고.
 *
 * 실행:  node glyph.js
 */
...
s=open('stardust-lab.html',encoding='utf-8').read()
...
const FONT_HASH='93c8900949c68ea3';
...
const m=html.match(/font-family:'Black Han Sans'[\s\S]*?base64,([A-Za-z0-9+/=]+)\)/);
if(!m){ console.log('★ 제목 폰트를 못 찾았다'); process.exit(1); }
const hash=crypto.createHash('sha256').update(m[1]).digest('hex').slice(0,16);
if(hash!==FONT_HASH){
