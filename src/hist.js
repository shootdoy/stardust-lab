...
function hCat(){
  ...
  const add=(n,r,s,g)=>{
    ...
    /* 기믹(g)도 담는다 — 다이맥스포켓몬 판의 보스를 거르는 데 쓴다 (v3.57.0).
       ★4 이하는 기믹 데이터가 없어 null 이다. */
    seen.add(k); out.push({n:String(n), r:String(r), s:String(s), g:g||null});
  };
  POOL.forEach(p=>add(p.n,p.r,p.s,p.g)); SUBS.forEach(x=>add(x.n,x.r,x.s,x.g));
  return out;
}
const HRORD='65432R';
...
function renderHPick(){
  ...
  const searching=!!q;
  /* ⚠ **다이맥스포켓몬 판의 보스는 다이맥스 기술이 있는 태그뿐이다** (12장 · v3.57.0 잭 지정).
     후보가 11장이라 성급·탄으로 또 가르면 빈 목록이 자주 나와 «눌러도 안 바뀐다» 로 보인다 —
     그래서 한 목록으로 보여 주고 두 스위치를 감춘다 (foeModal 과 같은 규칙).
     ⚠ **선물 칸은 거르지 않는다** — 선물은 판 종류를 가리지 않고 받는다.
     ⚠ 막지는 않는다 — 목록에 없는 이름을 검색하면 «그대로 ★N / 성급 모름» 버튼이 뜬다.
       현장에서 우리 데이터에 없는 것이 나올 수 있다 (홍나숭 사례). */
  const dmaxBoss = hSlot==='b' && hCur.m==='다맥';
  /* 검색 중에는 거르개를 감춘다 — 안 감추면 «눌러도 목록이 안 바뀐다» 로 보인다 (foeModal 규칙) */
  document.getElementById('hRank').hidden = searching || dmaxBoss;
  ...
  hs.hidden = searching || dmaxBoss || hRank==='R' || onSets.length<2;
  document.querySelectorAll('#hSet button').forEach(b=>{
    b.hidden = !onSets.includes(b.dataset.s);
    b.setAttribute('aria-pressed', b.dataset.s===hSet);
  });
  const cat=hCat(), list=[];
  /* ⚠ 검색에도 **똑같이** 건다 — 안 걸면 검색으로 우회해 넣을 수 있어 거른 의미가 없다
     (설정의 상대 탄 스위치와 같은 규칙). */
  const pool = dmaxBoss ? cat.filter(t=>t.g==='다이맥스') : cat;
  if(searching){
    for(const t of pool) if(foeMatch(t.n,q)) list.push(t);
    list.sort((a,b)=>HRORD.indexOf(a.r)-HRORD.indexOf(b.r)||a.n.localeCompare(b.n,'ko'));
  } else if(dmaxBoss){
    list.push(...pool);
    list.sort((a,b)=>HRORD.indexOf(a.r)-HRORD.indexOf(b.r)||a.n.localeCompare(b.n,'ko'));
  } else {
    for(const t of pool) if(t.r===hRank && (hRank==='R'||t.s===hSet)) list.push(t);
    list.sort((a,b)=>a.n.localeCompare(b.n,'ko'));
  }
  const hint=document.getElementById('hPickHint');
  hint.hidden=false;
  hint.textContent = searching
    ? `«${q}» 검색 — ${dmaxBoss?'다이맥스 태그 중 ':'성급 상관없이 '}${list.length}장`
    : dmaxBoss ? `다이맥스 기술이 있는 태그 ${list.length}장만 나옵니다`
  ...
  list.forEach(t=>{
    const b=document.createElement('button');
    /* 다이맥스 목록은 성급·탄이 섞여 있어 이름만으론 못 가른다 (1탄 마기라스 ★6 와
       2탄 마기라스 ★5 가 함께 있다). 검색 결과와 같은 형식으로 함께 적는다. */
    b.innerHTML = (searching||dmaxBoss)
...
function hAnalyze(){
  ...
  /* 지금 «마의 구간» — 마지막 ★6 보스 이후 몇 판인가. 6판을 넘겼으면 곧 나온다
     ⚠ 화면 문구는 **«마의 구간»** 이다 (v3.56.0 · 잭 지정 — 종전 «건조»).
       내부 이름 `dry` 는 그대로 둔다. 화면 낱말과 변수명이 다른 것을 헷갈리지 말 것.
